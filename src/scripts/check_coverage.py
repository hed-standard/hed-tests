...
class CoverageAnalyzer:
    ...
    def print_report(self):
        ...
        print(f"  Error codes with AI metadata: {summary['codes_with_ai_metadata']} "
    ...
    def generate_markdown(self, output_file: Path):
        ...
        for error_code in sorted(self.coverage_data.keys()):
            ...
            lines.append(
                f"| {error_code} | {data['test_cases']} | {test_types_str} | {ai_marker} | {schemas} |"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Check HED test coverage"
    )
    parser.add_argument(
        "--markdown",
        type=str,
